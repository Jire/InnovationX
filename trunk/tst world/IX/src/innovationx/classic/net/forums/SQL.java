...
public class SQL {
	...
	public static void createConnection() {
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost/highscores","highscores","testpass");
			stmt = con.createStatement();
			connectionMade = true;
			System.out.println("Connected to Highscores database");
		} catch (Exception e) {
	...
	public static void destroyConnection() {
		try {
			...
			System.out.println("[SQL]: Failed to receive SQL data.");
		}
	}
	
	public static boolean saveOnline(int online) {
		try {
			query("UPDATE online SET online="+online+" where ID='1'");
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		}
