...
public class ShopHandler {
    ...
    public int MaxShops = 101; //1 more because we don't use [0] !
    public int MaxShopItems = 101; //1 more because we don't use [0] !
    public int MaxInShopItems = 40;
    public int MaxShowDelay = 60;
    public int TotalShops = 0;
    public int[][] ShopItems = new int[MaxShops][MaxShopItems];
    public int[][] ShopItemsN = new int[MaxShops][MaxShopItems];
    public int[][] ShopItemsDelay = new int[MaxShops][MaxShopItems];
    public int[][] ShopItemsSN = new int[MaxShops][MaxShopItems];
    public int[] ShopItemsStandard = new int[MaxShops];
    public String[] ShopName = new String[MaxShops];
    public int[] ShopSModifier = new int[MaxShops];
    public int[] ShopBModifier = new int[MaxShops];
    ...
    public void process() {
        ...
        for (int i = 1; i <= TotalShops; i++) {
            ...
            if (DidUpdate == true) {
                for (int k = 1; k < Server.s.playerHandler.maxPlayers; k++) {
                    if (Server.s.playerHandler.players[k] != null) {
                        if (Server.s.playerHandler.players[k].IsShopping == true && Server.s.playerHandler.players[k].MyShopID == i) {
                            Server.s.playerHandler.players[k].UpdateShop = true;
                        }
                    }
                }
                DidUpdate = false;
